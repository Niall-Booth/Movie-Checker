...
//This establishes express and sets the Port being used. A selected port on Renders end when hosting through them or Port 3000 when 
//hosting locally when testing
const app = express();
...
//This runs when the Database Connection is lost due to idling to stop the whole Website from crashing.
...
/**
 * @description - This Function takes a given selection of Movies picks out 10 Random Movies (or however many are avaliable if 10 are not
...
//User
//This function renders the index page and displays 10 random movies to the user.
...
//This Function takes a Movie Name from the User and searches the Database for the Movie if the Movie is found then its infomation is
//displayed to the User, If the Movie isnt found then the User is told that the Movie wasnt found.
...
//This Function takes random search params from the User and uses them to query the Database for an Array of Movies. Then 10 random Movies
//are selected from this Array to make a new Array of randomMovies which are then displayed to the User on the Index Page.
...
//This Function renders the Page the User will use to add new Movies to the Database.
app.get("/add", (req, res) => {
    res.render("adding_page.ejs", {pages:pages, currentPage:"Add"});
})

//This Function takes all the Infomation entered about the new Movie from the User and attempts to enter it into the Movies Database.
//If the adding fails because the Movie already exists this is displayed to the User. Otherwise, the User to told it was a success.
...
//Admin
//This Function renders the Page for the user to login to the Admin Section.
app.get("/login", (req, res) => {
    res.render("login_page.ejs", {pages:pages, currentPage:"Login"});
})

//This Function takes the inputed Username and Password. Then it checks to see if the User exists if they do then it checks to see if the 
//given Password and the Stored Passwords match. If they do then the User is taken to the Admin Page.
...
//This Function querys the Database change_logs for all currently Store Logs. Each Log is given the Movie Number for the Movie in the Log.
//Then all the Logs are displayed to the User on the rendered Logs Page.
...
//This Function runs when the Server is up and logs to the Console the port the Server is running on so that I know that the Server is 
//working correctly.
app.listen(port, () => {
